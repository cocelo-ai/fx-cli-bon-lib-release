...
git clone -b "v${lib_ver}_py${py_ver}_${arch}" git@github.com:cocelo-ai/fx-cli-bon-lib-release.git
```
...
```bash
sudo apt install ./fx-cli-bon_${lib_ver}_py${py_ver}_${arch}.deb
```

사용 중인 환경에서 `apt install`이 정상적으로 동작하지 않는 경우, 아래 명령어를 사용합니다.

```bash
sudo dpkg -i fx-cli-bon_${lib_ver}_py${py_ver}_${arch}.deb
